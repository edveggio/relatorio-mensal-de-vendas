...
ggplot(data = Q1SalesSum, mapping = aes(x = Q1SalesSum$Ano, y = Q1SalesSum$Rendaper1000, label = Q1SalesSum$Rendaper1000))+
  ...
  xlab("Ano") +
  ggtitle("Renda em Q2 de cada ano" )

```
  
  Começamos comparando os semestres para tentar englobar a sazonalidade que já percebemos nas nossa vendas. A primeira vista só atingimos cerca de 10% das vendas do semestre até agora. Seria Abril um mês fraco e que recuperariamos nos outros meses do semestre?
...
MonthSalesSum <- MonthSales %>% 
 ...
 AbrilSalesSum <- MonthSalesSum %>% 
  filter(Data_de_fechamento == 4 & Ano != 2017)
...
ggplot(data = AbrilSalesSum, mapping = aes(x = AbrilSalesSum$Ano, y = AbrilSalesSum$Rendaper1000, label = AbrilSalesSum$Rendaper1000))+
  ...
  ylab("Renda em milhares de R$") +
  xlab("Ano") +
  ggtitle("Renda em Abril de cada ano" )
```
  
  Os dados nos ostram que não. Em abril ja vendemos de 3 a 4 vezes o que fizemos em 2020. Precisamos dar um gás nas vendas ou não conseguiremos nem manter o nível do nosso ano anterior. Certamente fomos impactados pela Pandemia.
  
```{r  Q1Sales, echo=FALSE}
QuarterSales <- pipe_FunildeVendas%>%
  filter(Fase == "Ganho Fechado")
...
```
  ...
  Ainda mais porque no primeiro semestre tivemos um bom desempenho mas não o nosso melhor.
  
```{r AcessosIlupi, echo=FALSE, message=FALSE, warning=FALSE, results = "hide"}
visitas_Ilupi <- read_csv("visitas Ilupi.csv", skip = 20, col_names = c("Dia", "Usuarios"))
...
visitas_Ilupi_Agregado$Mes <- month(visitas_Ilupi_Agregado$Data)
  ...
  ggplot() + 
    geom_line(data = visitas_Ilupi_Agregado, aes(x = Mes, y = Usuarios, color = Ano,), size = 2) + 
...
Visitas_Site_Pris <- read_csv("visitas Pris.csv", skip = 19, col_names = c("Dia", "Usuarios"))
...
```
  ...
  O acesso aos nossos sites caíram tanto Pris quanto Ilupi, voltando a níveis do fim do ano passado.
