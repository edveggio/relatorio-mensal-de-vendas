...
```{r echo=FALSE}
DT::datatable(tarefas_com_ploblemas, options = list(
...
Fluxo_de_caixa_2020_v29_projecao <- read_delim("D:/Ed Velho/Documents/Relatorio mensal de vendas - Pris/06-Jun/Fluxo de caixa 2020 v29 - projecao.csv", 
    ";", escape_double = FALSE, locale = locale(decimal_mark = ","), 
    trim_ws = TRUE, skip = 4)

names(Fluxo_de_caixa_2020_v29_projecao)[4] <-paste("categoria")

Fluxo_de_caixa_2020_v29_projecao<- Fluxo_de_caixa_2020_v29_projecao[,4:length(names(Fluxo_de_caixa_2020_v29_projecao))]

Fluxo_de_caixa_2020_v29_projecao <- Fluxo_de_caixa_2020_v29_projecao[c(-2,-3)]

projecao.as.numeric <- function(x){
  x <- as.numeric(gsub(",", ".", gsub("\\.", "", x)))
}

meses_custo <- Fluxo_de_caixa_2020_v29_projecao[,2:length(names(Fluxo_de_caixa_2020_v29_projecao))]

meses_custo <- apply(meses_custo, 2, projecao.as.numeric)

Fluxo_de_caixa_2020_v29_projecao[,2:length(names(Fluxo_de_caixa_2020_v29_projecao))] <- meses_custo

custosMKT <- Fluxo_de_caixa_2020_v29_projecao %>% 
...
sum_custos_MKT_this_month <- sum_custos_MKT$`mai/19`


new_clients_this_month <- MonthSales %>% 
  filter(between("Data de fechamento", today() - days(30), today()))
